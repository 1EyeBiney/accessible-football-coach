...
GameController in engine/controller.js is the only thing the UI talks to for a game: start, pending (what input is needed next), suggestion for each side, callOffense, callDefense, specialTeamsChoices and callSpecial (the fourth down flow: punt, field goal, go for it, or a fake), substitutions, advance, reports, situationLine, halftime, final, and the play log. It returns strings and plain objects, never DOM.
