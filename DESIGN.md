...
Status note, session 2 (August 27, 2026). Built without a fourth full coordinator; see DESIGN_PROPOSALS.md proposal 3 for the reasoning. The fourth-down flow (punt, field goal, go for it, or a fake) is a decision about the coach's own possession, so it is gated by offenseMode alone, the same three delegation levels as any other offensive call, with no dependence on defenseMode. The opponent's own fourth down and any kneel-out in victory formation are never a decision the coach is asked about, whatever the delegation setting.

Status note, session 4 (August 29, 2026). The coin toss and the kickoff are built, per Brian's play notes in ISSUES.md. Every game opens with the toss: the coach calls it in the air (H or T, always asked whatever the delegation settings - it is his moment and happens once), and a winner chooses receive, defer to the second half, or kick; there is no defend-an-end option yet because the engine has no wind or field direction for it to mean anything, and offering a choice that changes nothing would be dishonest. Every kickoff is then a call in the fourth-down grammar: deep, squib, pooch, or onside for the kicking team, regular return or hands team for the receiving one, gated by offenseMode - always asked in full control, only when an onside kick is genuinely in play (fourth quarter, inside four minutes, down one to sixteen) when the coordinator stops him for the big ones, never when fully delegated. Onside kicks have a real recovery mechanic: roughly one in five against a regular return unit, half that against a hands team sent on for exactly this. The computer coach uses the same deterministic desperation window on both sides of the ball.
...
Status note, session 4 (August 29, 2026). Brian accepted DESIGN_PROPOSALS.md proposal 4 (the lightest way to play). Two of its consequences are live now: "Enter always works, no prompt anywhere requires scheme knowledge to accept" is a permanent build gate alongside the accessibility criteria, and the controller counts, per game, the decisions the coach made himself against the ones his staff made for him (c.decisions, riding through saves), the seed the career's legacy tally and the identity traits will grow from. Intent calling - this section's "priorities in plain terms" level applied to game day - and the career accounting itself wait for the career milestones.
...
Status note, session of August 26, 2026. The hunch model now exists as engine/staff.js. Every staff member keeps a belief store for the game, fed only by the events on a play result and by pre-game scouting, and both automatic coaches now decide from those beliefs rather than from raw counters. Evaluation sets a per-matchup bias that is drawn once a game and never averages away, plus how many looks the coordinator needs before he speaks; Communication sets how long after that he takes to say it. Two changes were needed to make it work and both are argued in DESIGN_PROPOSALS.md: the quarterback now works the concept's progression in order rather than always finding the most open receiver, and defensive staffs now have coverage tendencies by down and distance for the offense to read. Measured over three hundred games, a good coordinator names the receiver who really is winning about sixty-nine percent of the time against about forty percent for a poor one. Turning that read into points is currently worth only about a point and a half a game, which is an open question recorded in the proposals file.
